    ...
    return (
        <div className="space-y-10 animate-in fade-in duration-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <MetricCard
                    title="Last Checkup"
                    value={healthRecords[0] ? new Date(healthRecords[0].recorded_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : 'N/A'}
                                ...
                                <div key={vaccine.id || idx} className="relative group">
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                        <div>
                                            ...
                                            <p className="text-micro text-muted-foreground font-bold uppercase tracking-widest mt-0.5 italic">Method: {vaccine.administration_method || 'Standard'}</p>
                                        </div>
                                ...
                                <div key={record.id} className="p-4 bg-muted/30 rounded-xl border border-border/50 group hover:border-primary/30 transition-colors">
                                    <div className="flex justify-between items-start mb-2">
                                        <div className="flex flex-col">
                                            <span className="text-xs font-bold text-foreground uppercase tracking-tight">{record.symptom_observation || 'Observation'}</span>
                                            <span className="text-micro text-muted-foreground font-bold uppercase tracking-widest mt-0.5">
                                                {new Date(record.recorded_at).toLocaleDateString()}
                                            </span>
                                        </div>
                                        <StatusBadge 
                                            status={record.diagnosis_category === 'infectious' ? 'danger' : 'warning'} 
                                            size="sm" 
                                            label={record.diagnosis_category || 'General'}
                                        />
                                    </div>
                                    <p className="text-micro text-muted-foreground font-bold uppercase tracking-widest italic line-clamp-2 mt-2">{record.treatment_administered || 'No treatment recorded'}</p>
                                    <div className="mt-3 pt-3 border-t border-border/50 flex items-center gap-2">
                                        <div className="w-5 h-5 rounded-full bg-primary/10 flex items-center justify-center text-primary">
                                            <Icon name="UserIcon" size={10} />
                                        </div>
                                        <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Recorded by {record.profiles?.first_name || 'System'}</span>
                                    </div>
                                </div>
                            ))
                        ) : (
